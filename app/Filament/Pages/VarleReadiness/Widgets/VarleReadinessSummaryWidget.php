    ...
    protected function getStats(): array
    {
        ...
        return [
            ...
            Stat::make('Missing barcode', (string) $summary['missing_barcode'])
                ...
                ->url($metrics->productsFilterUrl(['barcode_status' => ['value' => 'some_variants_missing_barcode']])),
            Stat::make('Missing category', (string) $summary['missing_category_mapping'])
                ...
                ->url($metrics->productsFilterUrl(['category_status' => ['value' => 'missing']])),
            Stat::make('Missing variant images', (string) $summary['missing_variant_images'])
                ...
                ->url($metrics->productsFilterUrl(['image_status' => ['value' => 'some_exportable_variants_missing_image']])),
            Stat::make('No images', (string) $summary['no_images'])
                ->color($summary['no_images'] > 0 ? 'danger' : 'success')
                ->url($metrics->productsFilterUrl(['image_status' => ['value' => 'no_images']])),
            Stat::make('Out of stock blocked', (string) $summary['out_of_stock_no_backorder'])
                ...
                ->url($metrics->productsFilterUrl(['stock_status' => ['value' => 'out_of_stock_blocked']])),
            Stat::make('Backorder exportable', (string) $summary['backorder_exportable'])
                ->url($metrics->productsFilterUrl(['stock_status' => ['value' => 'mixed_stock_backorder']])),
            Stat::make('Default delivery rule', (string) $summary['vendor_delivery_missing'])
                ->url($metrics->productsFilterUrl(['vendor_delivery_rule' => ['isActive' => false]])),
